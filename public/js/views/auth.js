...
function renderLoginForm() {
  ...
  authScreen.innerHTML =
    '<div class="auth-card">' +
      ...
      '<button type="button" class="auth-google-btn" id="google-signin-btn" onclick="handleGoogleSignIn()">' +
        ...
        '<span id="google-signin-text">Continuar con Google</span>' +
      '</button>' +
...
async function handleGoogleSignIn() {
  ...
  var btnText = document.getElementById('google-signin-text');
  var errorEl = document.getElementById('auth-error');
  ...
  try {
    ...
    var result = await window.insforge.auth.signInWithOAuth('google', {
    ...
    if (result.error) {
      ...
      if (btnText) btnText.textContent = 'Continuar con Google';
      return;
    }

    // SDK auto-redirects to Google. If we reach here, something unexpected happened.
    if (btnText) btnText.textContent = 'Redirigiendo...';
  } catch (err) {
    showAuthError('No se pudo iniciar sesion con Google. Intenta de nuevo.');
    if (btn) {
      btn.disabled = false;
      btn.classList.remove('loading');
    }
